        ...
        Try
            ...
            For Each Rows As System.Windows.Forms.DataGridViewRow In Table2Check.SelectedRows
                ...
                If Me.Label16.Text <> "0" Then
                    ...
                    FeatureNeedToAdd.ArrayLocation = SelectedIF(j).ArrayLocation
                    FeatureNeedToAdd.EntityMember = SelectedIF(j).EntityMember
                    FeatureNeedToAdd.FeatureName = SelectedIF(j).FeatureName
                    FeatureNeedToAdd.HiddenArcCount = SelectedIF(j).HiddenArcCount
                    FeatureNeedToAdd.HiddenLineCount = SelectedIF(j).HiddenLineCount
                    FeatureNeedToAdd.HiddenStatus = SelectedIF(j).HiddenStatus
                    FeatureNeedToAdd.ListLoop = SelectedIF(j).ListLoop
                    FeatureNeedToAdd.MiscProp(0) = SelectedIF(j).MiscProp(0)
                    FeatureNeedToAdd.MiscProp(1) = SelectedIF(j).MiscProp(1)
                    FeatureNeedToAdd.MiscProp(2) = SelectedIF(j).MiscProp(2)
                    FeatureNeedToAdd.MiscProp(3) = SelectedIF(j).MiscProp(3)
                    FeatureNeedToAdd.MiscProp(4) = SelectedIF(j).MiscProp(4)
                    FeatureNeedToAdd.MiscProp(5) = SelectedIF(j).MiscProp(5)
                    FeatureNeedToAdd.OriginAndAddition(0) = SelectedIF(j).OriginAndAddition(0)
                    FeatureNeedToAdd.OriginAndAddition(1) = SelectedIF(j).OriginAndAddition(1)
                    FeatureNeedToAdd.OriginAndAddition(2) = SelectedIF(j).OriginAndAddition(2)
                    FeatureNeedToAdd.OriginAndAddition(3) = SelectedIF(j).OriginAndAddition(3)
                    FeatureNeedToAdd.OriginAndAddition(4) = SelectedIF(j).OriginAndAddition(4)
                    FeatureNeedToAdd.OriginAndAddition(5) = SelectedIF(j).OriginAndAddition(5)
                    FeatureNeedToAdd.OriginAndAddition(6) = SelectedIF(j).OriginAndAddition(6)
                    FeatureNeedToAdd.OriginAndAddition(7) = SelectedIF(j).OriginAndAddition(7)
                    FeatureNeedToAdd.Planelocation = SelectedIF(j).Planelocation
                    FeatureNeedToAdd.Pline = SelectedIF(j).Pline
                    FeatureNeedToAdd.SequenceSolidBound = SelectedIF(j).SequenceSolidBound
                    FeatureNeedToAdd.SequenceSolidHidden = SelectedIF(j).SequenceSolidHidden
                    FeatureNeedToAdd.SolidArcCount = SelectedIF(j).SolidArcCount
                    FeatureNeedToAdd.SolidLineCount = SelectedIF(j).SolidLineCount
                    FeatureNeedToAdd.SolidLineInBoundCount = SelectedIF(j).SolidLineInBoundCount
                    FeatureNeedToAdd.SurfaceName = SelectedIF(j).SurfaceName
                    FeatureNeedToAdd.VirtualLineCount = SelectedIF(j).VirtualLineCount
                ElseIf Me.Label17.Text <> "0" Then
                    ...
                    FeatureNeedToAdd.ArrayLocation = SelectedUF(j).ArrayLocation
                    FeatureNeedToAdd.EntityMember = SelectedUF(j).EntityMember
                    FeatureNeedToAdd.FeatureName = SelectedUF(j).FeatureName
                    FeatureNeedToAdd.HiddenArcCount = SelectedUF(j).HiddenArcCount
                    FeatureNeedToAdd.HiddenLineCount = SelectedUF(j).HiddenLineCount
                    FeatureNeedToAdd.HiddenStatus = SelectedUF(j).HiddenStatus
                    FeatureNeedToAdd.ListLoop = SelectedUF(j).ListLoop
                    FeatureNeedToAdd.MiscProp(0) = SelectedUF(j).MiscProp(0)
                    FeatureNeedToAdd.MiscProp(1) = SelectedUF(j).MiscProp(1)
                    FeatureNeedToAdd.MiscProp(2) = SelectedUF(j).MiscProp(2)
                    FeatureNeedToAdd.MiscProp(3) = SelectedUF(j).MiscProp(3)
                    FeatureNeedToAdd.MiscProp(4) = SelectedUF(j).MiscProp(4)
                    FeatureNeedToAdd.MiscProp(5) = SelectedUF(j).MiscProp(5)
                    FeatureNeedToAdd.OriginAndAddition(0) = SelectedUF(j).OriginAndAddition(0)
                    FeatureNeedToAdd.OriginAndAddition(1) = SelectedUF(j).OriginAndAddition(1)
                    FeatureNeedToAdd.OriginAndAddition(2) = SelectedUF(j).OriginAndAddition(2)
                    FeatureNeedToAdd.OriginAndAddition(3) = SelectedUF(j).OriginAndAddition(3)
                    FeatureNeedToAdd.OriginAndAddition(4) = SelectedUF(j).OriginAndAddition(4)
                    FeatureNeedToAdd.OriginAndAddition(5) = SelectedUF(j).OriginAndAddition(5)
                    FeatureNeedToAdd.OriginAndAddition(6) = SelectedUF(j).OriginAndAddition(6)
                    FeatureNeedToAdd.OriginAndAddition(7) = SelectedUF(j).OriginAndAddition(7)
                    FeatureNeedToAdd.Planelocation = SelectedUF(j).Planelocation
                    FeatureNeedToAdd.Pline = SelectedUF(j).Pline
                    FeatureNeedToAdd.SequenceSolidBound = SelectedUF(j).SequenceSolidBound
                    FeatureNeedToAdd.SequenceSolidHidden = SelectedUF(j).SequenceSolidHidden
                    FeatureNeedToAdd.SolidArcCount = SelectedUF(j).SolidArcCount
                    FeatureNeedToAdd.SolidLineCount = SelectedUF(j).SolidLineCount
                    FeatureNeedToAdd.SolidLineInBoundCount = SelectedUF(j).SolidLineInBoundCount
                    FeatureNeedToAdd.SurfaceName = SelectedUF(j).SurfaceName
                    FeatureNeedToAdd.VirtualLineCount = SelectedUF(j).VirtualLineCount
                End If
