...
Public Class ViewProcessor
    ...
    Public Sub MultipleViewProcessor(ByVal ListView As List(Of ViewProp), ByVal View As ViewProp, ByRef UnIdentifiedFeature As List(Of OutputFormat), _
        ...
        For Each GroupEntity As List(Of Entity) In View.GroupLoop
            ...
            CountEntity(GroupEntity, View, SLReference, SLBReference, VLReference, _
            ...
            For Each ViewTmp As ViewProp In ListView
                If IsNothing(ViewTmp.ViewTag) Then
                    For Each GroupEntity2 As List(Of Entity) In ViewTmp.GroupLoop
                            ...
                            For Each EntityTmp2 As Entity In GroupEntity2
                                If TypeOf EntityTmp2 Is Line Then
                                    ...
                                    If TypeOf EntityTmp Is Line Then
                                        LineTmp = New Line
                                        LineTmp = EntityTmp
                                        If (View.ViewType.ToLower = "front" And ViewTmp.ViewType.ToLower = "right") _
                                        Or (View.ViewType.ToLower = "right" And ViewTmp.ViewType.ToLower = "front") Then
                                            If GPCFrontRight(LineTmp.StartPoint, LineTmp.EndPoint, LineTmp2.StartPoint, LineTmp2.EndPoint, _
                                                             View.BoundProp(0), View.BoundProp(1), _
                                                             ViewTmp.BoundProp(0), ViewTmp.BoundProp(1)) = 1 Then
                                                count2loop = count2loop + 1
                                            End If
                                        End If

                                        If (View.ViewType.ToLower = "front" And ViewTmp.ViewType.ToLower = "top") _
                                        Or (View.ViewType.ToLower = "top" And ViewTmp.ViewType.ToLower = "front") Then
                                        ...
                                        If (View.ViewType.ToLower = "top" And ViewTmp.ViewType.ToLower = "right") Then
                                        ...
                                        If (View.ViewType.ToLower = "right" And ViewTmp.ViewType.ToLower = "top") Then
                                            If GPCTopRight(LineTmp2.StartPoint, LineTmp2.EndPoint, LineTmp.StartPoint, LineTmp.EndPoint, _
                                                           ViewTmp.BoundProp(0), ViewTmp.BoundProp(2), _
                                                           View.BoundProp(0), View.BoundProp(1)) = 1 Then
                                                count2loop = count2loop + 1
                                            End If
                                        End If
                                    ...
                                    ElseIf TypeOf EntityTmp Is Arc Then
                                        ...
                                        Or (View.ViewType.ToLower = "right" And ViewTmp.ViewType.ToLower = "front") Then
                                            If GPCFrontRight(ArcTmp.StartPoint, ArcTmp.EndPoint, LineTmp2.StartPoint, LineTmp2.EndPoint, _
                                                             View.BoundProp(0), View.BoundProp(1), _
                                                             ViewTmp.BoundProp(0), ViewTmp.BoundProp(1)) = 2 Then
                                        ...
                                        If (View.ViewType.ToLower = "top" And ViewTmp.ViewType.ToLower = "right") _
                                        Or (View.ViewType.ToLower = "right" And ViewTmp.ViewType.ToLower = "top") Then
                                        ...
                                        If (View.ViewType.ToLower = "right" And ViewTmp.ViewType.ToLower = "top") Then
                                            If GPCTopRight(LineTmp2.StartPoint, LineTmp2.EndPoint, ArcTmp.StartPoint, ArcTmp.EndPoint, _
                                                           ViewTmp.BoundProp(0), ViewTmp.BoundProp(2), _
                                                           View.BoundProp(0), View.BoundProp(1)) = 2 Then
                                                count2loop = count2loop + 0.5
                                            End If
                                        End If
                                    End If
                        ...
                        If count2loop > 4 Then
                            CountEntity(GroupEntity2, ViewTmp, SLCorresponding, SLBCorresponding, VLCorresponding, _
                            ...
                            If SLReference = 4 And SLBReference = 2 And SAReference = 0 And SLCorresponding = 3 _
                               And SLBCorresponding = 0 And HLCorresponding = 0 And VLCorresponding = 1 Then
                                ...
                                For Each EntityTmp2 As Entity In GroupEntity2
                                    TmpLine = New Line
                                    TmpLine = EntityTmp2
                                    For Each LineBB As Line In ViewTmp.BoundingBox
                                        If (PointOnline(TmpLine.StartPoint, LineBB.StartPoint, LineBB.EndPoint) = 2 And _
                                        PointOnline(TmpLine.EndPoint, LineBB.StartPoint, LineBB.EndPoint) <> 2) Or _
                                        (PointOnline(TmpLine.EndPoint, LineBB.StartPoint, LineBB.EndPoint) <> 2 And _
                                        PointOnline(TmpLine.StartPoint, LineBB.StartPoint, LineBB.EndPoint) = 2) Then
                            ...
                            ElseIf SLReference = 4 And SLBReference = 3 And SAReference = 0 And SLCorresponding = 2 _
                                ...
                                'rule-based for 4-side pocket features (blind or through) 
                            ElseIf (SLReference = 4 And SLBReference = 0 And SAReference = 4 And SLCorresponding = 1 _
                                ...
                                D1 = D1 + (2 * D4)
                                D2 = D2Temp + (2 * D4)
                                PolygonProcessor.GetArea(View.GroupLoopPoints(View.GroupLoop.IndexOf(GroupEntity)))
                                Origin = PolygonProcessor.GetCentroid(View.GroupLoopPoints(View.GroupLoop.IndexOf(GroupEntity)))
                                OriU = Origin.X - View.BoundProp(0)
                                OriV = Origin.Y - View.BoundProp(1)
                                Angle = AngleTmp
                                ...
                                'rule-based for 3-side pocket features
                            ElseIf SLReference = 4 And SLBReference = 1 And SAReference = 2 And SLCorresponding = 4 _
                                   And SLBCorresponding = 1 And HLCorresponding = 0 And VLCorresponding = 0 Then
                                Dim D1, D2, D3, D4, OriU, OriV, OriW, Angle As New Double
                                Dim Orientation As String = Nothing
                                Dim TmpLine As Line
                                Dim TmpArc As Arc
                                ...
                                'rule-based for 2-side pocket features
                            ElseIf SLReference = 4 And SLBReference = 2 And SAReference = 1 And SLCorresponding = 4 _
                                   And SLBCorresponding = 2 And HLCorresponding = 0 And VLCorresponding = 0 Then
                                ...
                                Select Case View.ViewType.ToLower
                                    Case "top", "bottom"
                                        If ViewTmp.ViewType.ToLower = "left" Or ViewTmp.ViewType.ToLower = "right" Then
                                            D3 = D3H
                                        ElseIf ViewTmp.ViewType.ToLower = "front" Or ViewTmp.ViewType.ToLower = "back" Then
                                            D3 = D3V
                                        End If
                                    Case "front", "back"
                                        If ViewTmp.ViewType.ToLower = "left" Or ViewTmp.ViewType.ToLower = "right" Then
                                            D3 = D3H
                                        ElseIf ViewTmp.ViewType.ToLower = "top" Or ViewTmp.ViewType.ToLower = "bottom" Then
                                            D3 = D3V
                                        End If
                                    Case "right", "left"
                                        If ViewTmp.ViewType.ToLower = "front" Or ViewTmp.ViewType.ToLower = "back" Then
                                            D3 = D3H
                                        ElseIf ViewTmp.ViewType.ToLower = "top" Or ViewTmp.ViewType.ToLower = "bottom" Then
                                            D3 = D3V
                                        End If
                                ...
                                'rule-based for long hole feature 
                            ElseIf SLReference = 2 And SLBReference = 0 And SAReference = 4 And SLCorresponding = 1 _
                                   And SLBCorresponding = 1 And HLCorresponding = 3 And VLCorresponding = 0 Then
                                Dim D1, D2, D3, D4, OriU, OriV, OriW, Angle As New Double
                                Dim Orientation As String = Nothing
                                Dim TmpLine As Line
                                Dim TmpArc As Arc
                                ...
                                'rule-based for blind slot features
                            ElseIf SLReference = 3 And SLBReference = 1 And SAReference = 2 And SLCorresponding = 3 _
                               And SLBCorresponding = 0 And HLCorresponding = 0 And VLCorresponding = 1 Then
            ...
            If IdentificationStatus = False Then
                ...
                If InputStatus = False Then
                    ...
                    CountEntity(GroupEntity, View, SLReference, SLBReference, _
